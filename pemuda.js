...
NamaOwner2 = settings.NamaOwner2
multi = true
nopref = false
...
try {
		...
		cmhit.push(command)
        mess = {
			wait: 'sabar bang lagi di proses (kalo gk muncul berati server error)',
		...
		const ownerNumberr2 = [`${NomorOwner2}@s.whatsapp.net`]
		...
		const isOwner2 = ownerNumberr2.includes(sender)
...
return reply(parse)
           ...
           if (budy.startsWith('>')){
if (!isOwner && !isOwner2)return reply(`Perintah ini tidak bisa Di lakukan oleh Jadibot sementara`)
...
if (budy.startsWith('$')){
if (!mek.key.fromMe && !isOwner && !isOwner2) return
qur = budy.slice(2)
exec(qur, (err, stdout) => {
...
if (budy.startsWith('=>')){
if (!isOwner && !mek.key.fromMe && !isOwner2) return
...
reply(String(e))
			...
			// MUTE
        if (isMuted){
            if (!isGroupAdmins && !isOwner && !mek.key.fromMe && !isOwner2) return
...
break
case 'stopjadibot':
if (!isOwner && !mek.key.fromMe && !isOwner2) return reply(mess.only.ownerB)
try {
reply('Oke')
...
encmediam = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
			...
			if (!isBotGroupAdmins) return reply(mess.only.Badmin)
				 ...
				 case 'nsfw':
	        if (!isGroup) return reply(mess.only.group)
			if (!isOwner && !isGroupAdmins && !isOwner2) return reply(mess.only.admin)
